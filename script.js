...
class IScene extends THREE.Scene{
    ...
    constructor(option){
        ...
        //キャンバス、スライダー、チェックボックス等の親要素
        this.block = document.createElement('div');
        document.body.appendChild(this.block);  //webページに親要素を追加

        //キャンバス
        this.canvas = document.createElement('canvas');
        this.block.appendChild(this.canvas); //親要素にキャンバスを追加

        //マウスイベント
        this.IInputReader = new IInputReader();

        //このキャンバス上でマウスを動かしたときの処理
        this.canvas.addEventListener('pointerdown',()=>this.IInputReader.IsMouseDown = true, false); 
        this.canvas.addEventListener('pointerup',()=>this.IInputReader.IsMouseDown = false, false);  
        this.canvas.addEventListener('pointermove',(event)=>this.IInputReader.mousemove(event),false);

        this.canvas.style.touchAction = 'none'; //キャンバス上でフリックしてもスクロールしない
        ...
        if(option.light=='4color'){ //4方向から4色のライト mathematica風
    ...
    Ianimation(){
        ...
        if(this.IInputReader.IsMouseDown && mouseIsPressed){
    ...
    IaddSlider(parameter, option){
        const defaultvalues = {min:0, max:100, step:1, init:null}; //スライダーのデフォルト値
        ...
        tmp.addEventListener('input',function(){    //スライダーを動かしたときに頂点座標を更新
    ...
    IaddAnimeObject(vtsfilename, indexfilename, option){
        const defaultvalues = {scale:1, material:0xffffff, wireframe:false};
        option = {...defaultvalues, ...option}; 
...
class IGraphicComplex{
    ...
    constructor(vtsfilename, indexfilename, scene, option){
        ...
        this.ready = false; //テキストファイル読み込み完了
        ...
        async function f3(){
            return new Promise((resolve)=>{
                ...
                if(that.option.material=='normal'){
                    ...
                    material = new THREE.MeshLambertMaterial({side:THREE.DoubleSide, color:that.option.material});  //単色
                }
                ...
                //スライダーを使ってポリゴンを部分的に非表示にするオプションを付けたとき
                if(that.option.range){

                    //ポリゴンインデックスが配列で格納されているとき2軸スライダーを追加
                    if(Array.isArray(that.index[0][0])){
                    ...
                    }else{  //ポリゴンインデックスが1列に格納されているときスライダー1つ追加
        ...
        //f1 f2 f3順番に処理（テキストファイルの読み込みが非同期処理であるため）
        f1().then(()=>f2()).then(()=>f3());
